...
""",
    ...
    with open(f"{out_dir}/job_info.yaml", "w") as file:
        ...
        file.write(f"    system_name: {system_name}-batch-size\n")
    ...
    df.to_csv(
        f"{model_name}-{args.system_name}-{num_gpus}-batch-size.csv",
        mode="a",
        header=False,
        index=False,
    )
    ...
    for batch_size in batch_sizes:
        for idx, prompt in prompts.items():
            ...
            for num_tokens_o in output_tokens_counts:
                ...
                for idx_gpus in range(num_gpus):
                    df[
                        f"Total Memory {idx_gpus}"
                    ] = nvidia_smi.getInstance().DeviceQuery("memory.total")["gpu"][
                        idx_gpus
                    ][
                        "fb_memory_usage"
                    ][
                    ...
                    df[
                        f"Used Memory {idx_gpus}"
                    ] = nvidia_smi.getInstance().DeviceQuery("memory.used")["gpu"][
                        idx_gpus
                    ][
                        "fb_memory_usage"
                    ][
